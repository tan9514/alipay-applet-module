 ...
 * @date 2021-08-26
 */
class AlipayAppletTemplateSeeder extends Seeder
{
    public function run()
    {
        if (Schema::hasTable('alipay_applet_template')){
            $info = DB::table('alipay_applet_template')->where('id', '>', 0)->first();
            if(!$info){
                $arr = $this->defaultInfo();
                if(!empty($arr) && is_array($arr)) {
                    $created_at = $updated_at = date("Y-m-d H:i:s");
                    foreach ($arr as $name => $item) {
                        DB::table('alipay_applet_template')->insert([
                            'group' => $item['group'],
                            'code' => $item['code'],
                            'title' => $item['title'],
                            'value' => $item["value"],
